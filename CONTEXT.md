...
- Repository status: **Sprints 1–4 implemented.** The active greenfield root contains the authenticated .NET/React walking skeleton, end-to-end vehicle/driver/customer catalogs, shipment CRUD plus filtered pagination, and shipment operation with joint resource assignment, availability/capacity rules and terminal lifecycle transitions. PostgreSQL migrations, automated tests, Docker Compose, CI, design documentation and the reoriented LaTeX thesis remain aligned. The previous implementation remains an archived reference oracle only.
...
- Current priority: Sprint 5, implementing the immutable shipment-event history with author and chronological visibility.
