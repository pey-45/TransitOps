...
- Repository status: **Sprints 1 and 2 implemented.** The active greenfield root contains the authenticated .NET/React walking skeleton plus end-to-end vehicle, driver and customer catalogs, PostgreSQL migrations, automated tests, Docker Compose, CI, design documentation and the reoriented LaTeX thesis. The previous implementation remains an archived reference oracle only.
...
- Current priority: Sprint 3, implementing shipment creation/editing/listing/detail, customer association, estimated load, date validation, filters and pagination as the next vertical slice.
