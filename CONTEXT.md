...
- Terraform CLI is now installed locally, the AWS provider has been initialized as `hashicorp/aws` `5.100.0`, the isolated `transitops-dev` AWS account is accessible through IAM Identity Center/SSO profile `transitops-dev`, the remote-state bootstrap has created `transitops-tfstate-142966787103-eu-west-1` plus `transitops-tfstate-locks`, and a real AWS `terraform plan` succeeds for `infra/terraform/environments/dev` with HTTPS enabled for `api.dev.transitops.net`. No `apply` has been executed for the `dev` foundation/runtime stack yet.
...
- The `dev` runtime targets HTTPS on `api.dev.transitops.net` through Route53, ACM, and ALB when applied, but the cost-bearing runtime is currently destroyed for cost control. Re-applying it will recreate resources such as NAT Gateway, ALB, RDS, ECS, ECR, runtime secrets, and CloudWatch logs.
