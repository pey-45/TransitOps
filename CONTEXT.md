...
- Repository status: **Sprints 1–5 implemented.** The active greenfield root contains the authenticated .NET/React walking skeleton, end-to-end vehicle/driver/customer catalogs, shipment CRUD plus filtered pagination, shipment operation with resource assignment and terminal lifecycle, and immutable shipment-event histories with automatic/manual events, authenticated actor and chronological SPA visibility. PostgreSQL migrations, automated tests, Docker Compose, CI, design documentation and the reoriented LaTeX thesis remain aligned. The previous implementation remains an archived reference oracle only.
...
- Current priority: Sprint 6, implementing user administration, self-service password change and the operational summary.
- `ICurrentUser` is now the shared, testable way for application services to access the authenticated user id from the JWT `sub` claim; reuse it where Sprint 6 actions need actor identity.
- RN-04 is enforced by a service query over other planned/in-progress shipments. Two truly simultaneous assignments could both pass the check before either write commits; this low-probability window is accepted for the current small-operator scope and must be reviewed during Sprint 7 hardening (transactional locking or a PostgreSQL-specific constraint if warranted).
