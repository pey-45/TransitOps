...
- Repository status: **starting fresh.** The decision (2026-07-07) is to rebuild the application from scratch following the new iterative full-lifecycle methodology, keeping the previous-iteration code only as an archived reference oracle. The repository root currently contains **only planning documentation** (`README.md`, `CONTEXT.md`, `AGENTS.md`, `docs/`) and the `archive/`. There is no active application code, solution, tests, compose, or CI at root yet — those are recreated fresh as the sprints progress.
...
- The frontend project name/location, build tooling, and test stack are open decisions for the first implementation sprint (S1).
- The backend/frontend project names, folder layout, and solution structure are open — they are created fresh in S1, informed by (not copied from) the archived reference.
