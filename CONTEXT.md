...
- Repository status: **Sprints 1–3 implemented.** The active greenfield root contains the authenticated .NET/React walking skeleton, end-to-end vehicle/driver/customer catalogs, and shipment creation/editing/detail plus filtered pagination, with PostgreSQL migrations, automated tests, Docker Compose, CI, design documentation and the reoriented LaTeX thesis. The previous implementation remains an archived reference oracle only.
...
- Current priority: Sprint 4, implementing vehicle/driver assignment, availability rules, capacity warning and shipment lifecycle transitions as the next vertical slice.
