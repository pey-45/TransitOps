...
- Repository status: **Sprints 1–6 implemented; RF-01…RF-14 integrated; thesis consolidated through S6.** The active .NET/React application covers authentication/bootstrap, catalogs, shipment CRUD and filters, assignment/lifecycle, immutable event traceability, admin-only user management, self-service password change and a period-aware operational summary. PostgreSQL migrations, automated tests, Docker Compose, CI, design documentation and the reoriented LaTeX thesis remain aligned. The thesis now includes the resulting S2–S6 design, real TikZ diagrams and a regenerated PDF; S7 system validation, accessible deployment and final conclusions remain deliberately pending. The previous implementation remains an archived reference oracle only.
