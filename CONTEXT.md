...
- The `dev` runtime targets account `661000947340` and should initially use the ALB DNS/HTTP fallback unless `transitops.net` is present/delegated in that account. Re-applying it will recreate resources such as NAT Gateway, ALB, RDS, ECS, ECR, runtime secrets, and CloudWatch logs.
...
- Local Terraform files that may contain environment-specific or sensitive values (`terraform.tfvars`, `backend.hcl`, `.terraform/`) are ignored by Git. The current `dev` plan uses a temporary planning-only database password and must not be treated as a production secret if `apply` is run later.
