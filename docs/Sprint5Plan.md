...
**Dentro**: entidad `ShipmentEvent` + migración incremental; captura de la identidad del usuario autenticado en la capa de servicio; alta y consulta de eventos por envío; registro automático desde `CreateAsync`, `AssignAsync`, `UnassignAsync` y `ChangeStatusAsync`; línea de tiempo en el detalle del envío con formulario de alta sin recargar; pruebas backend y frontend; actualización de `DataModel.md`, Postman, Roadmap, CONTEXT, README y AGENTS.
