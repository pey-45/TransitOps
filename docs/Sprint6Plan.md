...
- **El mismo razonamiento vale para desactivar a alguien desde RF-04**: su token sigue funcionando hasta caducar (máximo 60 minutos por defecto, `JwtOptions.ExpirationMinutes`). RN-13 dice que un usuario desactivado no puede acceder, y esto lo cumple **con retardo**. Merece una nota explícita en la memoria: es una consecuencia real de la arquitectura sin estado elegida en S1, no un descuido, y la mitigación (validar `IsActive` en cada petición) es precisamente el tipo de endurecimiento que S7 puede evaluar.
...
6. **Memoria LaTeX** (`tfg/memoria/`): sección de S6 en `contido/desarrollo_iterativo.tex`, filas en `contido/validacion.tex`, párrafo en `contido/resultados.tex`, y `anexos/trazabilidad.tex` con `RF-03, RF-04, RF-14` pasando a evidencia real — con lo que **la tabla de trazabilidad queda completa**, que es un resultado citable en sí mismo. Capturas: panel de resumen, administración de usuarios, el 409 de último administrador y el formulario de contraseña. **Dos puntos merecen desarrollo escrito**: RN-12 como regla que tres operaciones distintas pueden violar (con la tabla del §1), y el análisis honesto del token que sobrevive a la desactivación — reconocer una limitación de la arquitectura elegida y razonar su mitigación es exactamente lo que un tribunal valora.
...
5. **Flujo funcional en el navegador**: login como administrador → Inicio muestra el resumen → dar de alta un operador → cerrar sesión → entrar como el operador → **«Usuarios» no aparece y `/usuarios` redirige** → cambiar su propia contraseña → volver a entrar con la nueva. Capturas para la memoria.
