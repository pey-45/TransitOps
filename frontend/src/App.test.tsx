...
describe('envíos', () => {
  ...
  it('rejects an invalid pickup without leaving the form pending', async () => {
    localStorage.setItem('transitops.session', JSON.stringify(session)); const fetchMock = vi.fn((input: RequestInfo | URL) => String(input).endsWith('/customers') ? response({ data: [], requestId: 'customers' }) : Promise.reject(new Error('Unexpected request'))); vi.stubGlobal('fetch', fetchMock); const rendered = renderAt('/envios/nuevo'); await screen.findByLabelText('Referencia')
  ...
  it('shows assignment controls and blocks starting an unassigned planned shipment', async () => {
    localStorage.setItem('transitops.session', JSON.stringify(session)); vi.stubGlobal('fetch', operationMock()); renderAt('/envios/shipment-1')
  ...
  it('assigns both resources and repaints the detail without reloading the shipment', async () => {
    ...
    localStorage.setItem('transitops.session', JSON.stringify(session)); const fetchMock = operationMock(shipment, assigned); vi.stubGlobal('fetch', fetchMock); renderAt('/envios/shipment-1'); const user = userEvent.setup()
  ...
  it('shows a successful capacity warning as a notice rather than an error', async () => {
    ...
    localStorage.setItem('transitops.session', JSON.stringify(session)); vi.stubGlobal('fetch', operationMock(shipment, assigned)); renderAt('/envios/shipment-1'); const user = userEvent.setup()
    await user.selectOptions(await screen.findByLabelText('Vehículo'), 'vehicle-1'); await user.selectOptions(screen.getByLabelText('Conductor'), 'driver-1'); await user.click(screen.getByRole('button', { name: 'Asignar' }))
  ...
  it('hides every operation for a delivered shipment', async () => {
    ...
    localStorage.setItem('transitops.session', JSON.stringify(session)); vi.stubGlobal('fetch', operationMock(delivered)); renderAt('/envios/shipment-1')
