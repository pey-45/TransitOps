...
\section{Uso de los TFG de referencia}
\label{sec:anexo-referencias-tfg}

Los TFG de referencia revisados muestran una estructura habitual en memorias técnicas: resumen, contexto, requisitos, diseño, implementación, validación, planificación, costes, conclusiones y anexos. También muestran que una memoria de entre 85 y 110 páginas no se consigue solo con describir código, sino desarrollando el razonamiento de diseño, las alternativas, la validación y el material complementario.

La ampliación de esta memoria sigue esa línea, pero adaptada a TransitOps. El contenido añadido se centra en explicar por qué se eligieron ASP.NET Core, PostgreSQL, Docker, Terraform, ECS Fargate, RDS, ALB, CloudWatch y GitHub Actions; cómo se diseñó el contrato HTTP; cómo se gestiona configuración y secretos; cómo se opera un despliegue; y cómo se relacionan requisitos con evidencias.

No se incorporan textos de los TFG de referencia. Su utilidad es estructural: ayudan a calibrar la densidad esperada de una memoria final y a detectar que la versión anterior de TransitOps era demasiado compacta para el nivel de detalle que suele verse en trabajos entregables. La nueva versión mantiene el mismo proyecto, pero explica con mayor profundidad las decisiones tomadas.

\section{Mantenimiento posterior}
\label{sec:anexo-mantenimiento-posterior}

Aunque el TFG se cierra con un entorno \texttt{dev} efímero, el repositorio queda preparado para mantenimiento posterior. La primera tarea razonable sería repetir una validación AWS final si se necesitasen evidencias nuevas en una fecha posterior, siguiendo el mismo flujo de recreación, validación y destroy ya documentado.

La segunda tarea sería ejecutar una validación AWS final si se necesitan evidencias recientes. Esa validación debe seguir el flujo ya documentado: recrear \texttt{dev}, publicar imagen, ejecutar migración, escalar ECS, validar HTTPS y login, revisar observabilidad y destruir. No conviene dejar el entorno vivo solo por comodidad, porque el diseño del proyecto asume coste temporal controlado.

La tercera tarea sería revisar permisos IAM si el proyecto evolucionase más allá del TFG. El rol OIDC de GitHub es aceptable para un entorno académico y efímero, pero un entorno productivo debería restringir acciones, recursos y condiciones con mayor precisión. Del mismo modo, una evolución a producción exigiría backups RDS automáticos, retención de logs más definida, alarmas con notificación confirmada, WAF, rotación de secretos y posiblemente autoscaling.

La cuarta tarea sería ampliar la observabilidad. El proyecto ya permite correlacionar peticiones, pero no incluye trazas distribuidas ni métricas de negocio. Si TransitOps creciese, sería útil medir número de transportes creados, errores por endpoint, duración de operaciones críticas y eventos de dominio relevantes. Estas métricas permitirían pasar de una observabilidad centrada en infraestructura a una observabilidad también orientada al producto.
