...
Eventos de ECS mostrando fallo de despliegue con \path{sprint7-missing-3dca035}, error \texttt{CannotPullContainerError} y posterior recuperación con \path{sprint7-good-3dca035}.
\vspace{1.2cm}
\end{minipage}}
\caption{Evidencia de rollback automático y recuperación manual.}
\label{fig:placeholder-ecs-rollback-events}
\end{figure}
...
\vspace{1.2cm}
\textbf{Placeholder de captura}\\
Snapshot manual de RDS, instancia restaurada temporal \path{transitops-dev-db-restore-sprint7} y tarea ECS \path{--migrate-only} \path{7f53d7309acf42349137213deca05080} finalizada con exit code \texttt{0}.
\vspace{1.2cm}
\end{minipage}}
\caption{Evidencia de restore real de RDS.}
\label{fig:placeholder-rds-restore}
\end{figure}
