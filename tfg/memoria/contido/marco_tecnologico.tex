...
Este capítulo describe el conjunto de tecnologías seleccionadas para el proyecto, explicando el papel de cada una dentro de la solución y justificando su elección frente a alternativas disponibles en el mercado. Las decisiones tecnológicas no son independientes entre sí: forman un ecosistema coherente en el que backend, persistencia, contenedores, infraestructura cloud, CI/CD y observabilidad se complementan para cubrir los objetivos del trabajo con la menor complejidad operativa razonable. Las herramientas principales utilizadas son ASP.NET Core, PostgreSQL con Entity Framework Core, Docker, Terraform, AWS, GitHub Actions y CloudWatch.
...
ASP.NET Core se eligió frente a otras opciones como Node.js con Express o NestJS, Spring Boot o FastAPI por varias razones. En primer lugar, ofrece tipado estático e integración nativa con Entity Framework Core, lo que resulta adecuado para un dominio con reglas de negocio moderadamente complejas. En segundo lugar, el sistema de inyección de dependencias y la configuración por entornos facilitan la separación entre código y configuración sensible, un requisito explícito del proyecto. Por último, el conocimiento previo del framework por parte del alumno permitió dedicar el tiempo de aprendizaje al ciclo cloud en lugar de al lenguaje.
...
PostgreSQL se eligió frente a MySQL porque ofrece soporte nativo a índices parciales con condiciones, lo que resulta imprescindible para implementar restricciones de unicidad sobre registros activos con borrado lógico. Frente a bases de datos NoSQL como MongoDB, el modelo relacional encaja mejor con el dominio del proyecto, donde las entidades tienen relaciones claras y se requiere integridad referencial. SQLite no es viable en despliegues cloud con múltiples réplicas de contenedor.
...
Docker es el estándar de facto para la contenerización en entornos cloud y cuenta con la mayor integración nativa con los servicios de AWS, en particular con ECR y ECS Fargate. Frente a herramientas alternativas como Podman, la compatibilidad total con el daemon Docker simplifica el pipeline CI/CD y elimina configuraciones adicionales en los runners de GitHub Actions. Docker Compose se usa en desarrollo frente a alternativas más complejas como Minikube o k3d porque ofrece una configuración más ligera y suficiente para un entorno de un solo nodo.
...
Terraform se eligió frente a AWS CloudFormation por su menor verbosidad, su modelo de módulos reutilizables y su capacidad de gestionar recursos de múltiples proveedores en la misma configuración. Frente a AWS CDK, Terraform es agnóstico al proveedor cloud y dispone de un ecosistema de módulos más amplio. Pulumi fue descartado por ser una herramienta menos extendida y porque su modelo de scripts de infraestructura en código de aplicación añadiría fricción en un contexto académico.
...
AWS se eligió como plataforma cloud frente a Microsoft Azure o Google Cloud Platform por varias razones. En primer lugar, AWS es el proveedor cloud con mayor cuota de mercado, lo que aporta valor formativo directo al trabajo. En segundo lugar, los servicios específicos utilizados (ECS Fargate, RDS, ACM, Route 53, Secrets Manager, CloudWatch) forman un ecosistema maduro y bien documentado que cubre las necesidades del proyecto sin herramientas adicionales. Azure y GCP ofrecen servicios equivalentes, pero requerirían adaptar la configuración Terraform y los flujos de despliegue a un ecosistema diferente sin aportar una ventaja técnica significativa para los objetivos del TFG.
...
GitHub Actions se eligió como plataforma de CI/CD frente a Jenkins porque no requiere gestionar infraestructura de servidor de integración continua. Frente a GitLab CI, la integración es nativa al repositorio sin necesidad de migración. Su soporte incorporado para OIDC con AWS simplifica considerablemente el flujo de credenciales, eliminando la necesidad de almacenar claves de larga duración como secretos del repositorio.
...
CloudWatch se utiliza como plataforma de observabilidad frente a soluciones externas como Datadog, Grafana Cloud o New Relic porque está integrada de forma nativa con los servicios AWS desplegados, no añade licencia adicional y es suficiente para las necesidades de un entorno de desarrollo con un único servicio. La correlación de peticiones mediante header y logs JSON estructurados proporciona un nivel de trazabilidad adecuado para el alcance del proyecto. OpenTelemetry y trazas distribuidas quedan como línea futura para cuando el sistema crezca en número de servicios.
