...
Este capítulo cubre las decisiones de observabilidad y seguridad adoptadas en TransitOps. Se describen los endpoints de salud que permiten al balanceador de carga tomar decisiones sobre el tráfico, el sistema de logs JSON correlables en CloudWatch, el dashboard y alarmas operativas, las medidas de seguridad aplicadas en aplicación e infraestructura, las limitaciones asumidas deliberadamente, y la revisión de seguridad realizada en el Sprint 8. El objetivo de este capítulo es demostrar que el entorno \texttt{dev} es operable y auditable, no solo desplegable.
...
\begin{itemize}
  \item acepta \httpheader{X-Correlation-ID} si el cliente lo proporciona;
  \item genera un identificador nuevo si el header no existe;
  \item devuelve siempre \httpheader{X-Correlation-ID} en la respuesta;
  \item alinea \texttt{HttpContext.TraceIdentifier} con ese valor;
  \item abre un scope de logging con \tech{CorrelationId};
  \item registra por petición método, ruta, código HTTP, duración, usuario y rol cuando existen.
\end{itemize}

Esto permite seguir una petición concreta desde la respuesta HTTP hasta los logs de CloudWatch. Además, los errores gestionados por el middleware de excepciones heredan el mismo identificador, por lo que el campo de error devuelto al cliente y el log del servidor quedan correlados. La Figura~\ref{fig:cloudwatch-logs-correlation} ilustra este mecanismo con una petición real de readiness y su entrada de log JSON asociada en CloudWatch.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{imaxes/cloudwatch-logs-correlation}
\caption{Evidencia de petición readiness con identificador de correlación y log JSON correlable en CloudWatch.}
\label{fig:cloudwatch-logs-correlation}
\end{figure}
...
Las alarmas usan \texttt{treat\_missing\_data} conservador para reducir ruido cuando \texttt{dev} está destruido o con \texttt{desired\_count = 0}. Si se configura \texttt{alarm\_email}, Terraform crea un topic SNS y una suscripción de email; si no se configura, las alarmas existen sin acciones. Durante la validación de Sprint 6 no se creó topic SNS porque no se proporcionó email. La Figura~\ref{fig:cloudwatch-dashboard} muestra el dashboard \path{transitops-dev-api} con los paneles de ALB, ECS, RDS y logs recientes creados automáticamente por Terraform.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{imaxes/cloudwatch-dashboard}
\caption{Evidencia del dashboard operativo de TransitOps con métricas de ALB, ECS, RDS y logs recientes.}
\label{fig:cloudwatch-dashboard}
\end{figure}
...
\begin{itemize}
  ...
  \item Correlación de errores y respuestas mediante \httpheader{X-Correlation-ID}, evitando exponer trazas internas al cliente.
\end{itemize}
