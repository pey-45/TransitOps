...
Las pruebas de sistema del Sprint 7 recorren los cuatro flujos de negocio con Playwright sobre la composición de contenedores. Sobre la sesión comprueban que la cookie no es legible desde \texttt{document.cookie}, evidencia observable del atributo \texttt{HttpOnly}, y que sobrevive a una recarga completa de la página. La colección ejecutable de la API añade la verificación directa de la cabecera \texttt{Set-Cookie}, incluidos \texttt{HttpOnly} y \texttt{SameSite=Strict}, y la reautenticación posterior al cambio de contraseña, cuya invalidación por versión de token cubren además las pruebas de servicio del backend.

Queda fuera de esa automatización el atributo \texttt{Secure}. La integración continua no dispone de terminación TLS y ejecuta la API con el entorno \texttt{Development}, en el que la aplicación emite deliberadamente la cookie sin esa marca para que el recorrido funcione por HTTP; \texttt{Secure} solo se activa fuera de \texttt{Development}. Su verificación es, por tanto, manual, y se realizó sobre el despliegue accesible con HTTPS: las herramientas del navegador muestran la cookie \texttt{transitops\_session} con las tres marcas ---\texttt{HttpOnly}, \texttt{Secure} y \texttt{SameSite=Strict}--- sobre la dirección pública del túnel. Se documenta de forma explícita porque implica que una de las propiedades endurecidas en el Sprint 7.3 se ejercita automáticamente en el único entorno donde permanece desactivada, y su comprobación depende de una inspección puntual y no de la suite.
