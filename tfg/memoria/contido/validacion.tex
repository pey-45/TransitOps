...
Queda fuera de esa automatización el atributo \texttt{Secure}. La integración continua no dispone de terminación TLS y ejecuta la API con el entorno \texttt{Development}, en el que la aplicación emite deliberadamente la cookie sin esa marca para que el recorrido funcione por HTTP; \texttt{Secure} solo se activa fuera de \texttt{Development}. Su verificación es, por tanto, manual y se realiza sobre el despliegue real con HTTPS siguiendo el procedimiento de despliegue. Se documenta de forma explícita porque implica que una de las propiedades endurecidas en el Sprint 7.3 se ejercita automáticamente en el único entorno donde permanece desactivada.
